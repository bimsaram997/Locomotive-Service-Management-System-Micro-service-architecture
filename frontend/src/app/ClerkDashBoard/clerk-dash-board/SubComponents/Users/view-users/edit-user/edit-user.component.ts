...
export class EditUserComponent implements OnInit {
 Roles: string[] = ['Supervisor', 'Service Manager', 'Clerk', 'Chief Engineer'];
  places = [
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, private cd: ChangeDetectorRef,
   private accessService: AccessService, private formBuilder: FormBuilder,) { }
 ...
 getOneUser(){
   this.accessService.getUser(this.data.id).pipe(first())
   .subscribe(
     res=>{
      if(res != undefined){
        console.log(res);
        ...
        this.editUserGroup.controls['userGender'].setValue(res[0].userGender);
        this.editUserGroup.controls['appointmentDate'].setValue(res[0].appointmentDate);
        this.editUserGroup.controls['address'].setValue(res[0].address);
        this.editUserGroup.controls['userWorks'].setValue(res[0].userWorks);
        this.editUserGroup.controls['userMobile'].setValue(res[0].userMobile);
 ...
 updateUser(){
    if (window.confirm('Are you sure?')) {
      this.accessService.editUser(this.editUserGroup.value)
      .subscribe(res=>{
       if (res !=null) {
          swal({
            title: 'Saved!',
